		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。</div>';
		            html += '               <span class="z-type">&nbsp;允许类型：<font color="red">'+para.fileType+'</font></span>';
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
				    ...
				    html += '							<img class="upload_image" src="/static/admin/Js/zyUpload/skins/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
					...
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">确认上传文件</button>';
					html += '		</div>';
					html += '		<div class="filesize">单文件最大 <font color="red">'+(Math.round(para.fileSize * 100 / (1024 * 1024)) / 100 ).toString()+'</font> MB</div>';
					html += '		<div id="uploadInf" class="upload_inf"></div>';
					html += '	</div>';
					html += '</form>';
				}
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				var type = newStr.split(".")[0].split("").reverse().join("");
				// 处理不同类型文件代表的图标
				var fileImgSrc = "/static/admin/Js/zyUpload/skins/images/fileType/";
				if(type == "rar"){
