if(!bdshare){
	var bdshare = {
		...
		init:function(callback){
			//加载百度分享代码
			if(!bdshare.status) {
				$.ajax({type:'GET',url:'https://cache.kuajingyan.com/plugin/baidu_share/static/api/js/share.js?cdnversion='+~(-new Date()/36e5),dataType: 'script',ifModified:true,cache:true,success:function(){bdshare.status=true;callback&&callback();}});
			}else{
				callback && callback();
