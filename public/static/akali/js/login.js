...
$(function () {
	...
	$("#sub-login-JSEncrypt").click(function () {
		...
		var publickey = null;
		var web_privatekey = null;
		var sign = null;
		
		$.post("/index/crypt_api/get_key", {}, function(res){
			...
			// 用户端私钥
			web_privatekey = res.data.web_privatekey;
			// 签名
			sign = res.data.sign;
			...
			// 数据加密
			usercode = encrypt.encrypt(usercode);
			password = encrypt.encrypt(password);
			/* console.log('前端');
			console.log('服务器公钥'+publickey);
			console.log('用户端私钥'+web_privatekey);
			console.log('用户名'+usercode);
			...
			$.ajax({
				...
				success: function (json) {
					...
					if (json.code == 200) {
						...
						// 设置私钥
						decryptor.setPrivateKey(web_privatekey);
						$.each(json.data, function(index, value){
							console.log(decryptor.decrypt(value));
						});
