	    	...
	    	if (json.code == 200){
	    		...
	    		if(json.data.list != ''){
	    			$.each(json.data.list, function(key, val) {
    					...
    					html += '<a class="personTop" href="/index/user/collegeOrder/order_id/'+val.ap_id+'">';
						html += '<div class="name">'+val.add_user+'</div>';
						html += '<div class="state">'+val.order_status_name+'</div>';

						html += '</a>';
						html += '<a class="partyMess" href="/index/user/collegeOrder/order_id/'+val.ap_id+'">';
						html += '<div class="img">';
						html += '<img alt="'+ val.activity_name_short +'" src="'+ val.activity_thumb +'">';
						html += '</div>';
						html += '<div class="messTxt">';
						html += '<div class="title">'+ val.activity_name_short;
						if(val.at_id == '8') {
						...
						html += '</div>';
						html += '</a>';
						html += '<a class="ticketBox" href="/index/user/collegeOrder/order_id/'+val.ap_id+'">';
						html += '<ul>';
						...
						if(val.order_status == '0'){
							html += '<div class="cz">';
							html += '<a class="yellow" href="/index/college/confirm/order_id/'+val.ap_id+'">马上支付</a>';
						...
						/* else if(val.order_status == '1'){
                            html += '<div class="cz">';
                            if(val.pay_status == 1){
                                if(val.need_pay == 1 && val.college_invoice && val.college_invoice.status == 1){
                                    html += '<a style="margin-left:5px;background: #ddd;color: #fff;" class="layui-btn layui-btn-xs layui-btn-warm" target="_blank" href="'+val.college_invoice.pdfUrl+'">已开发票</a>';
                                    //html += '<a style="" class="layui-btn layui-btn-xs layui-btn-warm DoRed" onclick="DoRed('+val.ap_id+',1)">退发票</a>';

                                }else if(val.need_pay == 1 && val.college_invoice && val.college_invoice.status == 2){
                                    ...
                                    html += '<a style="" class="layui-btn layui-btn-xs layui-btn-warm" href="/college/invoice/'+val.ap_id+'/1">开发票</a>';
	    			...
	    			html += '<li class="no_msg">您还没有报名课程</li>';
	    		}
	    		$(".data_ul").html(html);
...
function refund_order(id){
    ...
    $.get("/college/refund-order?id=" + id, function(result) {
        layer.open({
            type: 1,
            title: '确认退款',
            area: ['90%', '300px'],
