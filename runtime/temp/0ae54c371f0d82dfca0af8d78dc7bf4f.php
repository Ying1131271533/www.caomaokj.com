<?php /*a:2:{s:46:"../application/usezan/view/activity\index.html";i:1624506765;s:45:"../application/usezan/view/public\header.html";i:1650422517;}*/ ?>
<!DOCTYPE html>
...
<div class="mainbox animated fadeInRight">
	<div class="mainnav_title">
	    <a class="on">内容列表</a>
        <a class="add-edit " href="<?php echo url('activity/add'); ?>">[<b class="uzfont uzico-add"></b>活动添加]</a>
	    <a class="bth-rand fr" href="javascript:location.replace(location.href);" title="刷新"><b class="uzfont uzico-shuaxin"></b></a>
    </div>
<table  class="search_table" width="100%">
	<tr>
		<td class="search">
			<form action="<?php echo url('activity/index'); ?>" method="get">
				...
				<select class="catid" name="ain">
					<option value="0" <?php if($keys['ain'] == '0'): ?>selected<?php endif; ?>>活动状态</option>
					<option value="1" <?php if($keys['ain'] == '1'): ?>selected<?php endif; ?>>已结束</option>
					<option value="2" <?php if($keys['ain'] == '2'): ?>selected<?php endif; ?>>进行中</option>
				</select>
				<input type="hidden" name="tree" value="<?php echo htmlentities($tree_id); ?>"/>
				<input type="submit" value="<?php echo lang('chaxun'); ?>"  class="button" />
				<a href="<?php echo url('activity/index'); ?>" class="button">重置</a>
			</form>
		</td>
	</tr>
</table>
...
<div class="table-list">
		...
		<thead>
      	...
      	<tbody>
			...
			<tr>
				...
				<td align="center">
                    <a onclick="ajax_status(this)" data-id="<?php echo htmlentities($value['id']); ?>" data-url="<?php echo url('ajax/status', ['id' => $value['id'], 'value' => $value['popular'], 'field' => 'popular', 'db' => 'activity']); ?>" class="bth-a ajax-status <?php echo $value['popular']==1 ? htmlentities($value['popular']) : 'error-c'; ?>"></a>
                </td>
				...
				<td align="left">
					<a href="<?php echo url('activity/edit'); ?>?id=<?php echo htmlentities($value['id']); ?>&tree=<?php echo htmlentities($tree_id); ?>"><?php echo lang('edit'); ?></a> |
					<a data-url="<?php echo url('keyword/activityKeyword', ['id' => $value['id']]); ?>" class="keyword-article" href="javascript:;">关键词列表</a> | 
					<a href="javascript:confirm_delete('<?php echo url('activity/del'); ?>?id=<?php echo htmlentities($value['id']); ?>&tree=<?php echo htmlentities($tree_id); ?>')"><?php echo lang('delete'); ?></a>
				</td>
			</tr>
    ...
    <div class="btn">
	    <input  type="button" class="button" value="<?php echo lang('listorder'); ?>" onclick="myform.action='<?php echo url('activity/listorder'); ?>';$('#myform').submit();" />
    </div>
</div>
</form>
</div>
