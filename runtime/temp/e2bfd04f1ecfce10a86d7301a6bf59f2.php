<?php /*a:2:{s:45:"../application/usezan/view/service\index.html";i:1648538968;s:45:"../application/usezan/view/public\header.html";i:1650422517;}*/ ?>
<!DOCTYPE html>
...
<div class="mainbox animated fadeInRight">
	<div class="mainnav_title">
	    <a class="on">服务列表</a>
		<a class="add-edit " href="<?php echo url('service/add'); ?>">[<b class="uzfont uzico-add"></b>服务添加]</a>
	    <a class="bth-rand fr" href="javascript:location.replace(location.href);" title="刷新"><b class="uzfont uzico-shuaxin"></b></a>
    </div>
<table  class="search_table" width="100%">
	<tr>
		<td class="search">
			<form action="<?php echo url('service/index'); ?>" method="post">
				<input id="title" type="text" class="input-text" name="keyword" placeholder="服务名称搜索" value="<?php echo input('keyword'); ?>" />
				...
				<a href="<?php echo url('service/index'); ?>" class="button">重置</a>
			</form>
		</td>
	</tr>
</table>
...
<div class="table-list">
    <table width="100%" cellspacing="0">
		<thead>
            <tr>
				...
				<th width="200">名称</th>
				<th width="200">封面</th>
				<th width="right"><?php echo lang('createtime'); ?></th>
      	...
      	<tbody>
			...
			<tr height="70">
				...
				<td align="center">
                    <a onclick="ajax_status(this)" data-id="<?php echo htmlentities($value['id']); ?>" data-url="<?php echo url('ajax/status', ['id' => $value['id'], 'value' => $value['recommend'], 'field' => 'recommend', 'db' => 'service']); ?>" class="bth-a ajax-status <?php echo $value['recommend']==1 ? htmlentities($value['recommend']) : 'error-c'; ?>"></a>
                </td>

				<td align="center"><?php echo htmlentities($value['name']); ?></td>

				<td align="center"><img height="70" src="<?php echo !empty($value['image']) ? htmlentities($value['image']) : '/static/admin/Images/nodata.svg'; ?>" style="cursor:pointer;" onclick="open_img(this)" /></td>


				<td align="center"><?php echo htmlentities($value['create_time']); ?></td>

				<td align="center">
                    <a onclick="ajax_status(this)" data-id="<?php echo htmlentities($value['id']); ?>" data-url="<?php echo url('ajax/status', ['id' => $value['id'], 'value' => $value['status'], 'field' => 'status', 'db' => 'service']); ?>" class="bth-a ajax-status <?php echo $value['status']==1 ? htmlentities($value['status']) : 'error-c'; ?>"></a>
                </td>

                <td align="left">
                    <a href="<?php echo url('service/edit', ['id' => $value['id']]); ?>">修改</a> |
    ...
    <div class="btn">
	    <input  type="button" class="button" value="<?php echo lang('listorder'); ?>" onclick="myform.action='<?php echo url('service/sort'); ?>';$('#myform').submit();" />
    </div>
</div>
</form>
</div>
...
function del(id) {
    layer.confirm('确定要删除吗', function(index) {
        window.location.href = "<?php echo url('service/del'); ?>?id=" + id;
        layer.close(index);
    });
}
</script>