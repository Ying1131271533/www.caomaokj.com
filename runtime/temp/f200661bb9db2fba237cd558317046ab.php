<?php /*a:3:{s:45:"../application/usezan/view/article\index.html";i:1647586320;s:45:"../application/usezan/view/public\header.html";i:1650422517;s:49:"../application/usezan/view/public\menu_right.html";i:1574769382;}*/ ?>
<!DOCTYPE html>
...
<table  class="search_table" width="100%">
	<tr>
		<td class="search">
			<form action="<?php echo url('article/index'); ?>" method="post">
				...
				<select class="ispos" name="ispos">
					<option value="" <?php if($ispos == ''): ?>selected<?php endif; ?>>首页推荐</option>
					<option value="2" <?php if($ispos == '2'): ?>selected<?php endif; ?>>是</option>
					<option value="1" <?php if($ispos == '1'): ?>selected<?php endif; ?>>否</option>
				</select>
				<select class="catid" name="hot_spot">
					<option value="" <?php if($hot_spot == ''): ?>selected<?php endif; ?>>热门文章</option>
					<option value="2" <?php if($hot_spot == '2'): ?>selected<?php endif; ?>>是</option>
					<option value="1" <?php if($hot_spot == '1'): ?>selected<?php endif; ?>>否</option>
				</select>
				...
				<a href="<?php echo url('article/index'); ?>" class="button">重置</a>
			</form>
		</td>
	</tr>
</table>
...
<div class="table-list">
		...
		<thead>
      	...
      	<tbody>
			...
			<tr>
				...
				<td  width="50" align='center'><input name='listorders[<?php echo htmlentities($value['id']); ?>]' type='text' size='3' value='<?php echo htmlentities($value['listorder']); ?>' class='input-text-c'></td>

				<td align="center">
                    <a onclick="ajax_status(this)" data-id="<?php echo htmlentities($value['id']); ?>" data-url="<?php echo url('ajax/status', ['id' => $value['id'], 'value' => $value['ispos'], 'field' => 'ispos', 'db' => 'article']); ?>" class="bth-a ajax-status <?php echo $value['ispos']==1 ? htmlentities($value['ispos']) : 'error-c'; ?>"></a>
                </td>

				<td align="left"><img width="50" src="<?php echo !empty($value['thumb']) ? htmlentities($value['thumb']) : '/static/admin/Images/nodata.svg'; ?>" style="cursor:pointer;" onclick="open_img(this)" /></td>
				<td align="left"><?php echo htmlentities($value['title']); ?></td>
				<td align="center"><?php echo isset($category[$value['catid']]) ? $category[$value['catid']]['catname'] : '<font color="red">已删除</font>'; ?></td>

				<td align="center">
                    <a onclick="ajax_status(this)" data-id="<?php echo htmlentities($value['id']); ?>" data-url="<?php echo url('ajax/status', ['id' => $value['id'], 'value' => $value['hot_spot'], 'field' => 'hot_spot', 'db' => 'article']); ?>" class="bth-a ajax-status <?php echo $value['hot_spot']==1 ? htmlentities($value['hot_spot']) : 'error-c'; ?>"></a>
                </td>

				<td align="center"><?php echo htmlentities($value['createtime']); ?></td>
				<td align="center">
					<?php if($value['status'] == '1'): ?>
					<a class="bth-a ajax-status" onclick="StatusAjax(this);" data-href="<?php echo url('ajax/statusajax'); ?>?id=<?php echo htmlentities($value['id']); ?>&status=0"></a>
					<?php else: ?>
					<a class="bth-a error-c ajax-status" onclick="StatusAjax(this);" data-href="<?php echo url('ajax/statusajax'); ?>?id=<?php echo htmlentities($value['id']); ?>&status=1"></a>
					<?php endif; ?>
				</td>
				<td align="left">
					<a href="<?php echo url('article/edit'); ?>?id=<?php echo htmlentities($value['id']); ?>&tree=<?php echo htmlentities($tree_id); ?>"><?php echo lang('edit'); ?></a> |
	            	<a data-url="<?php echo url('keyword/articleKeyword', ['id' => $value['id']]); ?>" class="keyword-article" href="javascript:;">关键词列表</a> | 
					<a href="javascript:confirm_delete('<?php echo url('article/del'); ?>?id=<?php echo htmlentities($value['id']); ?>&tree=<?php echo htmlentities($tree_id); ?>')"><?php echo lang('delete'); ?></a>
				</td>
			</tr>
			<?php endforeach; ?>
      	</tbody>
    ...
    <div class="btn">
	    <input  type="button" class="button" value="<?php echo lang('listorder'); ?>" onclick="myform.action='<?php echo url('article/listorder'); ?>';$('#myform').submit();" />
    </div>
</div>
</form>
</div>
