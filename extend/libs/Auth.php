...
// +----------------------------------------------------------------------
// | Author: xiaoke <www.blogxuan.com/xiaoke>　
// +----------------------------------------------------------------------
namespace libs;
...
class Auth {
    ...
    public function __construct() {
        if (Config::get('sysadmin.auth_config')) {
            $this->config = array_merge($this->config, Config::get('sysadmin.auth_config')); //可设置配置项 auth_config, 此配置项为数组。
        }
    }
    /**
     * 检查权限
...
//            }
    ...
    public function getAuthList($uid, $type) {
        ...
        //读取用户组所有权限规则
        $rules = Db::name($this->config['auth_rule'])->where(new Where($map))->field('condition,name')->select();
        //循环规则，判断结果。
        $authList = [];   //
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) { //根据condition进行验证
                ...
                isset($condition) && $authList[] = strtolower($rule['name']);
            } else {
