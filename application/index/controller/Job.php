...
class Job extends Base
{
    ...
    {
        // halt($request->url());
        $job = ModelJob::getPageList(['status' => 1], 10, 'update_time');
        return view('', ['job' => $job]);
    }
    ...
    public function save_release()
    {
        ...
        if(!$city_name) return fail('发布地区有误');
        // 获取地区表id
        $city_value = ChinaPlaces::where('label', $city_name[2])->value('value');
        if(!$city_value) return fail('找不到发布地区');
        // 将省市区和地区表id加入数据中
        ...
        try {
            ...
            if(!$job) return fail('岗位保存出错1');
            
            $job_detail = $job->detail()->save($params);
            if(!$job_detail) return fail('岗位保存出错2');
    ...
    public function get_china_places()
    {
        ...
        // 地区数据分类
        $china_places = Cache::remember('china_places', $this->get_children($china_places), cache_time('one_month'));
